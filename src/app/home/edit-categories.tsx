...
export default function HomeEditCategoriesScreen() {
  const router = useRouter();
  const { homeExpenseCategories, addHomeExpenseCategory, updateHomeExpenseCategory, removeHomeExpenseCategory } = useSplitStore();
  ...
  const commitEdit = (original: string) => {
    ...
    if (t && t !== original) {
      mediumHaptic();
      updateHomeExpenseCategory(original, t);
    }
    setEditingCat(null);
  };
    ...
    <SafeAreaView style={s.safe} edges={['bottom']}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView style={s.scroll} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
          ...
          <Text style={s.hint}>
            Manage categories for home expenses like rent, utilities, and groceries.
          </Text>

          <Card padding={0} row={false} style={s.card}>
            {homeExpenseCategories.length === 0 && (
...
const s = StyleSheet.create({
  ...
  addInput: { flex: 1 },
  addBtn: { width: InputMetrics.height, height: InputMetrics.height, borderRadius: InputMetrics.radius, backgroundColor: C.card, justifyContent: 'center', alignItems: 'center' },
});
